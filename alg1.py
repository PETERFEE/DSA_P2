...
class SpamDecisionTree:
    ...
    def classify(self, email_text: str, nb_confidence: float) -> Tuple[str, Dict, float]:
        ...
        if nb_confidence >= self.HIGH_CONFIDENCE_THRESHOLD:
            ...
            if spam_score >= 0.75:
                ...
                return 'spam', reasoning, round((1000 * (time.perf_counter() - timestamp)), 2)
            elif spam_score < 0.50:
                reasoning['decision_path'].append('Low spam score contradicts')
                return 'ham', reasoning, round((1000 * (time.perf_counter() - timestamp)), 2)
            return 'spam', reasoning, round((1000 * (time.perf_counter() - timestamp)), 2)
